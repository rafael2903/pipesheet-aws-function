...
function getFormattedCards(cards, dateFieldsLabels) {
  ...
  return cards.map(({ node }) => ({
    ...
    ["Responsáveis"]: node.assignees
      ...
      .join(","),
    ["Criado em"]:
      node.created_at && new Date(node.createdAt).toLocaleString("pt-BR"),
    ["Atualizado em"]:
      node.updated_at && new Date(node.updated_at).toLocaleString("pt-BR"),
    ["Data de vencimento do card"]:
      node.due_date && new Date(node.due_date).toLocaleString("pt-BR"),
    ...buildFields(node),
...
function getHeaders(phases, fields, columns) {
  ...
  let headers = [...fieldsLabels, ...phasesHeaders];
